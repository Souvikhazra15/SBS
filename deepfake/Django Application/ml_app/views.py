...
class validation_dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        for i,frame in enumerate(self.frame_extract(video_path)):
            #if(i % a == first_frame):
            # Try to detect face using OpenCV, otherwise use full frame
            try:
...
def im_convert(tensor, video_file_name):
    ...
    # cv2.imwrite(os.path.join(settings.UPLOAD_IMAGES_DIR, video_file_name+'_convert_2.png'), image*255)
    return image
...
def predict(model,img,path = './', video_file_name=""):
  try:
    ...
    if 'out of memory' in str(e):
      ...
      gc.collect()
      raise Exception("GPU out of memory. Please try with fewer frames or restart the server.")
    raise
...
def plot_heat_map(i, model, img, path = './', video_file_name=''):
    ...
    image_name = os.path.join(settings.UPLOAD_IMAGES_DIR, heatmap_name)
...
def get_accurate_model(sequence_length):
    ...
    if len(sequence_model) > 1:
        ...
        raise FileNotFoundError(f"No model found for {sequence_length} frames in {models_dir}. Available models: {list_models}")
...
def index(request):
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            video_dest = os.path.join(settings.UPLOAD_VIDEOS_DIR, saved_video_file)
            with open(video_dest, 'wb') as vFile:
                shutil.copyfileobj(video_file, vFile)
            request.session['file_name'] = video_dest
...
def predict_page(request):
    if request.method == "GET":
        ...
        video_rel_path = os.path.join('videos', video_file_name)
        ...
        try:
            ...
            error_msg = f"Error: {str(e)}\n\nPlease ensure you have uploaded the correct model file with the right number of frames."
            context = {
        ...
        for i in range(sequence_length):
            ...
            image_path = os.path.join(settings.UPLOAD_IMAGES_DIR, image_name)
            img_rgb = pImage.fromarray(rgb_frame, 'RGB')
            img_rgb.save(image_path)
            preprocessed_images.append(image_rel_path)

            # Face detection and cropping using OpenCV cascade
            try:
                gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                face_cascade = cv2.CascadeClassifier(
                    cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
                ...
                if len(faces) == 0:
                    continue
                x, y, w, h = faces[0]
                frame_face = frame[y - padding:y + h + padding, x - padding:x + w + padding]
                
                # Check if frame_face is empty
            ...
            faces_found += 1
            faces_cropped_images.append(image_rel_path)
